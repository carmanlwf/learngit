<%@ Page Language="C#" AutoEventWireup="true" CodeFile="CardPassReset.aspx.cs" Inherits="CardHold_CardPassReset" %>
...
<head runat="server">
   ...
   <script type="text/javascript">
       function CloseWin(msg) {
           alert(msg);
           window.location.href = "CardPassReset.aspx";
       }
    </script>
    <script type="text/javascript">
        //2013-4-14 *ZSD
    ...
    <script type="text/javascript">
        ...
        //2013-5-7 *ZSD
        function PutCbValue() {
            btra = document.getElementById("Cbtradepassword").checked;
            bpas = document.getElementById("Cbpass").checked;

            if (btra && bpas)
                $("#CbTrue").val("登陆和交易");
            else if (btra && bpas == false)
                $("#CbTrue").val("交易");
            else if (btra == false && bpas)
                $("#CbTrue").val("登陆");
            else {
                $("#CbTrue").val("");
            }
...
<body onload="HideMe()">
    ...
    <div class="table_data form_table clearfix">
			<table>
				<tbody>
                    ...
                    vmode="not null" name="Mobile" vdisp="手机号码不能为空!" maxlength="11" /></td>
					</tr>
					<tr>
						<td class="xyfml">密码类型</td>
						<td class="xyfmr">
                <input id="Cbtradepassword" type="checkbox" runat="server" checked="checked" 
                                disabled="disabled" />交易密码&nbsp;&nbsp;
                <input id="Cbpass" type="checkbox" runat="server" disabled="disabled" />登录密码</td>
					</tr>
                    <tr>
						<td class="xyfml">摘 要</td>
						<td class="xyfmr">
                            <input type="text" class="xyin_s" style="width: 460px" id="memo" runat="server"
                    name="memo0" vdisp="卡号" maxlength="11" onclick="this.value = ''" onblur="javascript:if(this.value==''){this.value = '因客户忘记密码,持卡人本人申请密码重置.['+((new Date()).toLocaleString())+']'}"
                    value="因客户忘记密码,持卡人本人申请密码重置." /></td>
					</tr>
                    <tr>
						<td class="xyfml">提 示</td>
						<td class="xyfmr">1、该业务执行后,将重置客户交易密码为初始值“000000”
                <br />
                            2、交易密码指的是会员卡在消费/重置时的密码凭证,登录密码则是用户在线登录系统时的密码<br />
                ...
                <input id="btnSubmit" type="button" class="xybtn" value="提交" 
                            ...
                            <div style="display:none;"><asp:Button ID="Button1" runat="server" onclick="btnResetPass_Click" Text="Button" /></div>
                                </td>
					</tr>
				</tbody>
			</table>
    ...
    <input type="hidden" id="CbTrue" runat="server" vmode="not null" vdisp="请选择修改密码类型" />
    </form>
</body>
</html>
