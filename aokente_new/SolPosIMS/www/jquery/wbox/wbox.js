...
(function($) {
    ...
    $.fn.wBox = function(options) {
        ...
        var wboxi = "wBox";
        if (this.YQ.wBoxi != null)
            wboxi = wboxi + this.YQ.wBoxi;

        var wBoxHtml = '<div id="' + wboxi + '"  class="wBox" ><div class="wBox_popup"><table><tbody><tr><td class="wBox_tl"/><td class="wBox_b"/><td class="wBox_tr"/></tr><tr><td class="wBox_b"><div style="width:10px;">&nbsp;</div></td><td><div class="wBox_body">' + (_this.YQ.noTitle ? '' : '<table class="wBox_title"><tr><td class="wBox_dragTitle"><div class="wBox_itemTitle">' + _this.YQ.title + '</div></td><td width="20px" title="关闭"><div class="wBox_close"></div></td></tr></table> ') +
        ...
        this.showBox = function() {
            $("#" + wboxi + "_overlay").remove();
            $("#" + wboxi).remove();

            B = $("<div id='" + wboxi + "_overlay' class='wBox_hide'></div>").hide().addClass('wBox_overlayBG').css('opacity', _this.YQ.opacity).dblclick(function() {
                _this.close();
            ...
            if (_this.YQ.requestType && $.inArray(_this.YQ.requestType, ['iframe', 'ajax', 'img']) != -1) {
                ...
                if (_this.YQ.requestType === "img") {
                    ...
                    $.get(_this.YQ.target, function(data) {
                        con.html(data);
                        C.find('.' + wboxi + '_close').click(_this.close);
                        setPosition();
                    })

                }
                else {
                    ifr = $("<iframe name='" + wboxi + "Iframe' style='width:" + _this.YQ.iframeWH.width + "px;height:" + _this.YQ.iframeWH.height + "px;' scrolling='auto' frameborder='0' src='" + _this.YQ.target + "'></iframe>");
                    ifr.appendTo(con.empty());
                    ifr.load(function() {
                        try {
                            $it = $(this).contents();
                            $it.find('.wBox' + wboxi + '_close').click(_this.close);
